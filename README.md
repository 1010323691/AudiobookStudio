...
一个 Web 应用,在一个地方完成中文有声书的完整制作流程:**文本排版 → 分册切割 → 文本解析 → 段落混合检查 → 角色匹配检查 → 角色配音 → 音频合成 → 音频合并 → 音频分集**,外加开始(工作空间管理)与设置。
...
- **段落混合检查** — 解析与角色匹配检查之间的一道质检:把一条里混入多主体的段落(旁白+角色台词、或多角色台词)按原文顺序拆分为多条,并删除纯标点段落(本地确定性判定、不经 LLM);拆分须通过四道确定性校验(逐字拼回原文 / 角色名在白名单内 / ≥2 段 / ≥2 个不同主体),未通过的条目自动带上下文窗口重试一次,仍失败则保留原样;整批响应无法解析时立即原地重试一次(仍失败则整批保留);生成 `<文件基名>_checked.json`(原始 `.json` 永不改写);每批送检段数 / 上下文窗口与角色匹配检查共用
- **角色匹配检查** — 以混合检查产物 `_checked.json`(或基文件)为输入,用「±N 条上下文」逐条重判 `speaker`,就地更新 `<文件基名>_checked.json`(只改 `speaker` 字段,原始文件永不改写)
...
5. **段落混合检查**:在 `03_parsed_json/` 勾选已解析的 JSON(可多选)→ 点「开始混合检查」,每个文件作为独立任务:把混入多主体的段落按原文顺序拆分为多条、删除纯标点段落,生成 `<文件基名>_checked.json`(原始文件永不改写);可重跑——每次都从原始 `.json` 重新推导
6. **角色匹配检查**:勾选一个解析 JSON → 点「开始角色匹配检查」,逐条重判 `speaker`(带上下文窗口),就地更新 `_checked.json`(只改 `speaker`);输入自动取较新的 `_checked.json`(混合检查产物)或基文件
7. **角色配音**:选择一个解析 JSON(03_parsed_json/,或选「全部文件」合并整本书所有角色)→ 一键为角色生成声音画像(voice_config.json + 试听样本),也可仅处理新增角色或单角色重生成
8. **音频合成**:选择一个解析 JSON(03_parsed_json/)→ 启动批量合成(长时任务,进度 / 日志内联显示在页面)→ 在 `05_audio_chunk/` 产出逐行音频与 `manifest.json`
9. **音频合并**:按清单把逐行音频合并为 `06_audio_merge/cloned_audiobook.mp3`(换人 / 同人停顿可配)
10. **音频分集**:选择音频文件(通常是合并成品)→ 设置目标单集时长(默认 10:00)与命名格式 → 启动「停顿检测 + 切割」(长时任务,进度 / 日志内联显示在页面)→ 完成后可下载 zip 或输出到源音频同级「分集」文件夹
11. **设置**:配置工作空间、各模块参数(排版开关 / 每册字数 / 分集 / TTS 模型与停顿)、ffmpeg / ffprobe 路径、界面主题(跟随系统 / 亮 / 暗)、日志级别

所有产物按模块落在工作空间的固定子目录下:`01_input/`(排版文本)· `02_split_text/`(分册)· `03_parsed_json/`(解析 JSON,`<基名>.json` 原始 + `<基名>_checked.json` 检查产物)· `04_voice_profiles/`(角色声音)· `05_audio_chunk/`(合成片段)· `06_audio_merge/`(合并成品)· `07_output/`(最终分集);产物通过页面上的下载链接获取。
...
│   ├── engines/             # 核心算法:文本排版 / 分册 / 解析 / 两阶段检查(混合 / 角色匹配)/ 音频切割 / TTS(批量合成 / 合并 / 角色配音)
│   ├── core/                # 目录布局、持久化配置、任务系统、日志
│   ├── tests/               # pytest 测试套件(394 个测试)
