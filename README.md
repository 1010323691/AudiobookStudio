...
- **文本解析** — 「LLM → JSON」管线:粘贴小说原文,逐段调用 LLM 生成逐行标注脚本 `annotated_script.json`(角色 / 类型 / 演绎指令 / 停顿)
- **角色配音** — 为脚本中的角色生成声音画像(`voice_config.json` + 试听样本);支持一键全部、仅处理新增、或单角色按提示词重生成
- **音频合成** — 批量合成:单个长时任务驱动 `.venv-tts` 子进程,模型只加载一次,按 JSON 顺序合成全部行;单行失败只记录、不打断整批
- **音频合并** — 按清单把逐行音频无损合并为最终 `cloned_audiobook.mp3`,支持换人 / 同人停顿
...
4. **文本解析**:粘贴小说原文 → 配置 LLM(base_url / api_key / 模型)与生成参数 → 逐段调用 LLM,生成逐行标注脚本 `03_parsed_json/annotated_script.json`
5. **角色配音**:一键为脚本中出现的角色生成声音画像(voice_config.json + 试听样本),也可仅处理新增角色或单角色重生成
6. **音频合成**:启动批量合成(长时任务,进度 / 日志内联显示在页面)→ 在 `05_audio_chunk/` 产出逐行音频与 `manifest.json`
