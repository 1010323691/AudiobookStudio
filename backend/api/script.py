...
written to ``output/tts/annotated_script.json`` and served by the shared
``GET /api/files/download/tts/{name}`` route, so it drops straight into the TTS flow.
...
def result() -> dict:
    ...
    path = get_layout().output_tts / "annotated_script.json"
    if not path.is_file():
