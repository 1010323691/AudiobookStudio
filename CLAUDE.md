...
├── 02_split_text/  # 分册（每章一个文件 <base> 分册NN 第XXX章.txt；零章节时单个 <base> 全书.txt [+ <base>.zip]）+ 智能识别产物（每章一个 第 NNN 章 标题.txt [+ <base> 智能识别.zip]；两类产物并存、以最近一次为准）
...
| `book.py` | 分册（每章一册）+ 智能识别（章节结构机械修复）。`decode_buffer`（编码探测，**全后端共用**——`script.py` / `_common.py` 复用）、`analyze_text`、`make_chapter_filenames`（`<base> 分册NN 第XXX章.txt`，NN 与章号双宽独立）、`make_whole_book_filename`（`<base> 全书.txt`）、`chapter_content`（单章原文切片）、`check_chapter_sequence`、`EXPECTED_CHAPTER_FORMAT`（用户可见的章节格式文案）、`build_zip`、**`smart_repair`**（智能识别纯函数：重编号 1..N + 异常长章推断拆分 + 同号同内容章删除 + 自检，见阶段 1；`LONG_CHAPTER_RATIO=2.0` / `MIN_LENGTH_SAMPLES=5` / `MIN_BASELINE_CHARS=200` / `INFER_SNAP_TOLERANCE=0.5`）、`make_smart_filenames`（`第 NNN 章 标题.txt`，N≤1000 三位宽 / 否则四位宽，sanitize 后判重加 ` 2`/` 3` 后缀）、`_normalized_fingerprint`（切片去全部空白 sha256，重复章判定） |
...
单合并页：排版 → 自动章节分析 → 分册 / 智能识别（两条产出路径，产物并存于 `02_split_text/`）。所有环节均为**同步 HTTP**（无任务 / SSE）；页面状态全部**页面本地**（不经 project store），换选文件 / 重新排版 / 「重新上传原文」都**重置全部下游状态**（陈旧的分析 / 分册 / 智能识别结果绝不存活）；keep-alive 缓存页面、状态跨导航存活。
...
**分册** `POST /api/book/split {path, base?, as_zip?, whole_book?, smart?}`（`require_workspace`）：**严格按章节，每章恰好一个文件**——逐章 `chapter_content`（**原文单一字节切片**）`write_bytes` 进 `02_split_text/`；无章节 → **400**（唯一写整本的路径是显式 `whole_book=true`——零章节「不处理，按整本继续」，写单个 `<base> 全书.txt`）；可选 `build_zip`（`ZIP_STORED` 无压缩，UTF-8 文件名；两种模式都支持 → `<base>.zip`）。**`smart=true` = 按智能识别结果分册**（前端「分册（智能识别结果）」按钮）：重跑确定性 `smart_repair`（同输入 → 同结果），写出与 `/smart-split` **逐字节相同**的 `第 NNN 章 标题.txt` 文件（zip 名同样 `<base> 智能识别.zip`）——**不产生第二套 `分册NN` 命名**，02_split_text/ 里只有一套章节文件。
文件名：`<base> 分册{NN:zfill≥2} 第{XXX:zfill w}章.txt`（NN = 1 基**位置**顺序号，宽 `max(2, 章数位数)`；XXX = **原章号**，宽 `max(3, 最大章号位数)`——两宽独立驱动；不可解析章号保留原 `numStr` 不补零；`sanitize_file_name` 换非法字符）——重号靠 分册NN 天然消歧（两个第5章 → `分册01 第005章` / `分册02 第005章`）。
`check_chapter_sequence` 报告缺号/重号/乱序（**非阻断**，不改分册）：页面琥珀警告逐条列出具体问题（如「缺号：第3章之后缺 第4章」）+ 预期格式 `EXPECTED_CHAPTER_FORMAT` + 两个选择：「不处理，继续」（仅关闭警告，照常分册）/「重新上传原文」（整页重置，重跑排版 + 识别）。

**智能识别**（「开始分册」旁的按钮，用户手动触发；`sequence.hasIssues` 且未执行时琥珀 ring 高亮，**不自动执行**）`POST /api/book/smart-split {path, as_zip?}`（`require_workspace` 409 → 编码探测 400 → 无章节 400）：纯机械、**无 LLM**、确定性、输入只读副本；语义由用户拍板——**永远输出文件 + 修复报告，从不阻断**（疑似合并章即使扫不到标题行也按长度推断拆分并补号，重复章也删除，每个动作在报告中带置信级别供人工核验）。物理顺序 = 唯一真相；字数只用于定位推断切点（切点必吸附段落边界）；重编号恒为物理序 1..N；**文件内标题行保留原样**（不改正错号——价值在文件结构与命名，报告供追溯）。
链：`read_decoded_file` → `analyze_text` → `smart_repair(text, chapters)`（自检失败 → 400 诊断文案，**绝不带病输出**）→ 逐章 `chapter_content` + `write_bytes` 进 `02_split_text/`（与分册产物并存，「以最近一次产物为准」）→ `as_zip` → `build_zip` `<base> 智能识别.zip`。
修复机制：
...
`status`：全部动作 `kept` 且零警告才 `clean`，否则 `ok`；**对修复后结构再跑 → `clean`（幂等）**。响应 `{status, output_dir, file_count, files, zip_path?, chapters: [{seq, orig_num, orig_numStr, final_num, title, chars, actions, confidence}], report: {actions, warnings, removed: [{seq, num, numStr, title, kind}]}, baseline_chars, original_count, expected_format}`；actions ∈ `inferred_split`/`duplicate_kept`/`duplicate_truncated`/`renumbered`/`gap_absorbed`/`kept`，confidence = high/medium/low（逐章取最低）。
...
前端（`TextFormat.vue`，纯页面本地状态）：「智能识别」按钮（lucide `Sparkles`；输入同 analyze/split = 排版产物路径）+ 结果卡（状态/原章节数/最终章节数/基线 统计条；`report.warnings` 逐条琥珀 Alert；`report.removed` 逐条 info Alert；修复记录表 #/原号/新号/标题/操作/置信 Badge（`ScrollArea h-64`）；输出文件表 + zip 下载；脚注「文件内标题行保留原样（未改写正文）……推断拆分（低置信）与已删除章节请对照上方报告人工核对」）。**智能识别成功后覆盖原始排版识别**：`analysis` 状态整体替换为修复后的 1..N 结构（chapters/filenames/sequence 全按修复结果重建，`sequence.hasIssues=false` → 序号警告自然消失；章节表「分册文件」列显示智能识别文件名；章节分析卡头部注记「已按智能识别修复结果覆盖（原 N 章 → M 章）」）——此后「开始分册」按钮变「**分册（智能识别结果）**」（带 `smart=true` 调 `/split`），分册产物 = 与智能识别同一套 `第 NNN 章 标题.txt` 文件。`goNext()` guard 扩展为**分册或智能识别任一完成**（产物同落 `02_split_text/`，下游解析页直接列目录）。
