...
├── 02_split_text/  # 分册（每章一个文件 <base> 分册NN 第XXX章.txt；零章节时单个 <base> 全书.txt [+ <base>.zip]）
...
8. **确定性三引擎的不变量**（源自 JS 工具 1:1 移植，测试固化，用户依赖）：
   ...
   - `book.py`（←BookChunker）：章节**铺满整段文本**（首章 start=0、末章 end=len、`ch[i].end==ch[i+1].start`）；切分**只落章节边界**（**每章恰好一个文件**，`分册NN` 只是位置顺序号）；**从不重编号**（缺口保留原号，如 1,2,5 → `第001章`/`第002章`/`第005章` 三个文件）；**无章节 → 停止**（split 返 400；唯一例外是显式 `whole_book`——写单个 `<base> 全书.txt`）；**所有逐章文件拼接 == 原文**。
...
| `book.py` | 分册（每章一册）。`decode_buffer`（编码探测，**全后端共用**——`script.py` / `_common.py` 复用）、`analyze_text`、`make_chapter_filenames`（`<base> 分册NN 第XXX章.txt`，NN 与章号双宽独立）、`make_whole_book_filename`（`<base> 全书.txt`）、`chapter_content`（单章原文切片）、`check_chapter_sequence`、`EXPECTED_CHAPTER_FORMAT`（用户可见的章节格式文案）、`build_zip` |
...
单合并页：排版 → 自动章节分析 → 分册。两环节均为**同步 HTTP**（无任务 / SSE）；页面状态全部**页面本地**（不经 project store），换选文件 / 重新排版 / 「重新上传原文」都**重置全部下游状态**（陈旧的分析 / 分册结果绝不存活）；keep-alive 缓存页面、状态跨导航存活。
...
**分册** `POST /api/book/split {path, base?, as_zip?, whole_book?}`（`require_workspace`）：**严格按章节，每章恰好一个文件**——逐章 `chapter_content`（**原文单一字节切片**）`write_bytes` 进 `02_split_text/`；无章节 → **400**（唯一写整本的路径是显式 `whole_book=true`——零章节「不处理，按整本继续」，写单个 `<base> 全书.txt`）；可选 `build_zip`（`ZIP_STORED` 无压缩，UTF-8 文件名；两种模式都支持 → `<base>.zip`）。
...
不变量与测试：排版不变量 → `test_text.py`；分册不变量见核心约定 #8 → `test_book.py`。「前往下一步（文本解析）」→ `/script`（guard：分册已完成）。
...
## 测试（558 个，`backend/tests/`）

| 文件 | 固化的行为 |
|---|---|
| `test_text.py` | 排版：内容保持、幂等、空白/省略号/重复标点、孤立 ASCII 保护（URL）、对话/叙述分段、句断开关、章节隔离与正负样例、10 开关逐一、端到端样例 |
| `test_book.py` | 分册（每章一册）：章节铺满/真实章头/无章节即停（空文件名表）、**无损往返拼接 == 原文**（逐章切片）、字数排除换行（非 BMP 计 1）、命名格式（`分册NN 第XXX章`，NN 宽随章数、章号宽随最大章号，双宽独立）/补零/缺口不重编号/重号靠 NN 消歧/全书文件名（含清洗）/`EXPECTED_CHAPTER_FORMAT` 文案、编码探测（BOM/UTF-8/GB18030）、中文数字解析、序号体检、zip STORED 往返 |
