...
| `test_config.py` | 配置 round-trip、两文件模型（根只写指针/工作空间独立/不覆盖已有）、TTS 字段缺省（三模型 id、500/250、parallel_workers=1 ≠ batch_concurrency=4）、GenerationConfig 解析内检查缺省（revalidate_splits/delete_saying_tags = True、check_batch_size=20、check_context_window=4）、遗留字段保留 |
